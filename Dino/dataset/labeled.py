...
from Dino.generator.label_text import RawFileGenerator
...
class RowImageDataset(Dataset):
    "`ImRowImageDatasetageDataset` read data from raw file."
    ...
    def __init__(self,
        ...
        # 数据生成器，应该作为入参传入
        generator_mode = 'train' if is_training else ''
        if label_path is None:
            generator_mode = 'pred'
        else:
            generator_mode = 'eval'
        self.generator = RawFileGenerator(self.path, label_path, self.convert_mode, mode=generator_mode)

        dataset_length = len(self.generator) if dataset_length == -1 else dataset_length        
    ...
    def get(self, idx):
        image, label = self.generator.get(idx)
        if self.is_training:
            targets_dict = self.label_convertor.str2tensor([label.text])
        else:
            targets_dict = [label.text]
